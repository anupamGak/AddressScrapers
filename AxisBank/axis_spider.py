...
url = "http://m.axisbank.com/smartphone/WebForms/mBranch-Locator.aspx"
...
for i in  range(len(states['id'])):
	data = {
		...
		"locate_office$hdOffType" : "branches",
	...
	for j in range(len(cities['id'])):
		...
		for k in range(len(areas['id'])):
			...
			branch = {
				...
				"address" : page.xpath("//table[@class='result']//tr[3]/td[2]/div/text()")[0]
			}
			addr = page.xpath("//table[@class='result']//tr[3]/td[2]/div/text()")[0]
			branch['address'] = re.sub(r"\s{3,}", "", addr)
			try:
				branch['pincode'] = re.findall(rePin, addr)[0]
			except IndexError:
				branch['pincode'] = ""
			branch['sublocality'] = addr.split("\r\n")[1]
			branch['sublocality'] = re.sub(r"\s{3,}", "", branch['sublocality'])

			branchdump.append(branch)

with open('axis_addr.json', 'w') as f:
